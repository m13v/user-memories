...
- **Noise locations**: cities/states from insurance quoting, comparison shopping, or form testing (e.g. `country="AF"`, `city="Beaufort"` when user lives in SF)
...
- **Same phone, different formats**: `"+1 650-796-1489"`, `"(650) 796-1489"`, `"6507961489"`, `"1 (650) 796-1489"` — keep the international format with highest confidence, delete all others
- **Same email, different casing**: keep lowercase
- **Duplicate DOB entries**: identify the real one (consistent across sources) vs noise from insurance quoting
...
- **Wrong key assignment**: `company="Dmitrii Diakonov"` → delete (person name in company field), `company="38-4277177"` → delete (phone in company field)
- **Wrong confidence**: real DOB at 0.4 while fake one from insurance quoting at 0.6 → boost real one
- **Missing tags**: `phone` key without `phone` tag, `email` without `email` tag
- **Useful data in autofill keys**: if a valuable entry survived fast pass (e.g. `autofill:jobtitle="CTO"`), rename the key to a proper one (`job_title`) and fix tags
- **Broken supersession**: use `UPDATE memories SET superseded_by=NULL, superseded_at=NULL WHERE id=?` to unsupersede real values
...
- The user is **Matthew Diakonov** (also Matt Diakonov, Dmitrii Diakonov). Email: i@m13v.com. Keep his real data.
- Contacts stored as `contact:{Name}` or `linkedin:{Name}` are legitimate — keep them unless clearly garbage
...
- `autofill:*` entries are almost always duplicates of data already stored under proper keys — the fast pass deletes them all, but scan for secrets first
- `address_type_*` entries are always noise — name fragments and house numbers extracted into wrong fields
- When in doubt, check the `source` field for context
